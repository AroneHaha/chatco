    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">This action will deactivate the personnel account.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* Termination Type */}
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1.5">Termination Type <span className="text-red-400">*</span></label>
          <select
            value={terminationType}
            onChange={(e) => setTerminationType(e.target.value)}
            className="block w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-red-500 transition-colors [color-scheme:dark]"
          >
        ...
        {/* Reason Textarea */}
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1.5">Reason for Removal <span className="text-red-400">*</span></label>
          <textarea
            value={reason}
            ...
            className="block w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-red-500 transition-colors resize-none leading-relaxed"
          />
          <p className="text-[11px] text-gray-600 mt-1">This will be recorded in the Records & History tab.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-white/10">
          ...
          <button
            type="submit"
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-colors active:scale-[0.98]"
          >
            <Trash2 size={16} />
            Confirm Removal
          </button>
