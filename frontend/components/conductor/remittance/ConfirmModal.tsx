...
export default function ConfirmModal({ show, onClose, onConfirm, isRemitting, shiftInfo, gcashTotal, cashTotal, grandTotal, totalPassengers }: ConfirmModalProps) {
  ...
  // Reset to system total when modal reopens
  if (!touched && show) {
    // Keep the default value in sync until the conductor manually edits
  }
      ...
      <div className="relative w-full max-w-sm bg-[#0B1E33] border border-white/10 rounded-3xl overflow-hidden shadow-2xl animate-slide-up max-h-[90vh] overflow-y-auto">
        <div className="p-6 space-y-5">
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 space-y-2.5">
            ...
            <div className="flex justify-between text-sm"><span className="text-white/40 flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-400" />Cash (system-tracked)</span><span className="text-emerald-400 font-bold tabular-nums">{fmt(cashTotal)}</span></div>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              min="0"
              value={cashDeclared}
              onChange={(e) => { setCashDeclared(e.target.value); setTouched(true); }}
          ...
          <div className="flex gap-3">
            <button onClick={onClose} disabled={isRemitting} className="flex-1 py-3 rounded-xl bg-white/5 border border-white/10 text-white/60 text-sm font-semibold hover:bg-white/10 transition-all active:scale-[0.98] disabled:opacity-50">Cancel</button>
            <button onClick={() => onConfirm(declaredAmount)} disabled={isRemitting || declaredAmount < 0} className="flex-1 py-3 rounded-xl bg-[#1A5FB4] text-white text-sm font-bold shadow-lg shadow-[#1A5FB4]/25 hover:bg-[#165a9f] transition-all active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
