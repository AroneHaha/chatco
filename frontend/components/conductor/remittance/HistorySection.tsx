                ...
                {paginatedHistory.map((record, idx) => (
                  <div key={`${record.shiftId}-${idx}`} className="bg-[#071A2E] border border-white/[0.06] rounded-2xl p-4 space-y-2.5">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <div className="bg-white/[0.03] rounded-lg px-2.5 py-2"><p className="text-[8px] font-semibold text-emerald-400/40 uppercase tracking-wider">Cash</p><p className="text-sm font-extrabold text-emerald-400 mt-0.5 tabular-nums">{fmt(record.cashTotal)}</p></div>
                      <div className="bg-white/[0.03] rounded-lg px-2.5 py-2"><p className="text-[8px] font-semibold text-white/30 uppercase tracking-wider">Cashless</p><p className="text-sm font-extrabold text-[#62A0EA] mt-0.5 tabular-nums">{fmt(record.totalCashless)}</p></div>
                    </div>
                    <div className="flex items-center justify-between pt-1 border-t border-white/[0.04]"><span className="text-xs font-bold text-white/40">Total <span className="text-white font-extrabold tabular-nums">{fmt(record.gcashTotal + record.cashTotal + record.totalCashless)}</span></span><div className="flex items-center gap-2"><button onClick={() => openOfficialReport(record)} className="flex items-center gap-1 text-[#62A0EA]/70 hover:text-[#62A0EA] transition-colors active:scale-95"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg><span className="text-[10px] font-semibold">View</span></button><button onClick={(e) => { e.stopPropagation(); onPrintReport(record); }} className="flex items-center gap-1 text-emerald-400/70 hover:text-emerald-400 transition-colors active:scale-95"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0 1 10.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0 .229 2.523a1.125 1.125 0 0 1-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.318 0h1.091A2.25 2.25 0 0 0 21 15.75V9.456c0-1.081-.768-2.015-1.837-2.175a48.055 48.055 0 0 0-1.913-.247M6.34 18H5.25A2.25 2.25 0 0 1 3 15.75V9.456c0-1.081.768-2.015 1.837-2.175a48.041 48.041 0 0 1 1.913-.247m10.5 0a48.536 48.536 0 0 0-10.5 0m10.5 0V3.375c0-.621-.504-1.125-1.125-1.125h-8.25c-.621 0-1.125.504-1.125 1.125v3.659M18.75 7.131H5.25" /></svg><span className="text-[10px] font-semibold">Print</span></button></div></div>
                  </div>
                ))}
              </div>
