...
export default function ConductorDashboard() {
  ...
  // --- SHARED INNER UI ELEMENTS (Dark Mode to match Sidebar) ---
  const StatsGrid = () => (
    <div className="grid grid-cols-3 gap-3">
      <div className="bg-[#1A5FB4]/20 rounded-xl p-3 text-center border border-[#1A5FB4]/30">
        <p className="text-2xl font-extrabold text-[#62A0EA]">{mockData.passengers}</p>
        <p className="text-[10px] font-semibold text-[#62A0EA]/60 mt-1 uppercase">Passengers</p>
      </div>
      <div className="bg-purple-500/20 rounded-xl p-3 text-center border border-purple-500/30">
        <p className="text-2xl font-extrabold text-purple-400">₱{mockData.walletCollected}</p>
        <p className="text-[10px] font-semibold text-purple-400/60 mt-1 uppercase">Wallet</p>
      </div>
      <div className="bg-cyan-500/20 rounded-xl p-3 text-center border border-cyan-500/30">
        <p className="text-2xl font-extrabold text-cyan-400">₱{mockData.gcashCollected}</p>
        <p className="text-[10px] font-semibold text-cyan-400/60 mt-1 uppercase">GCash</p>
      </div>
    </div>
  );
  ...
  const CapacityToggle = () => (
    <div className="flex items-center justify-between bg-white/5 rounded-2xl p-4 border border-white/10">
      ...
      <button 
        ...
        className={`relative w-14 h-8 rounded-full transition-colors duration-300 ${isAvailable ? 'bg-[#1A5FB4]' : 'bg-gray-600'}`}
      >
  ...
  const ActionButtons = () => (
    <div className="space-y-2">
      <button className="w-full text-left px-4 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-medium text-white/80 transition-colors flex items-center gap-3">
        <span className="w-8 h-8 bg-[#1A5FB4]/30 text-[#62A0EA] rounded-lg flex items-center justify-center text-xs font-bold">₱</span>
        Log Expense (Gas, etc.)
      </button>
      <button className="w-full text-left px-4 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-medium text-white/80 transition-colors flex items-center gap-3">
        <span className="w-8 h-8 bg-orange-500/20 text-orange-400 rounded-lg flex items-center justify-center text-xs font-bold">!</span>
        Report Lost Item
      </button>
      <button className="w-full text-left px-4 py-3 bg-red-500/10 hover:bg-red-500/20 rounded-xl text-sm font-medium text-red-400 transition-colors flex items-center gap-3">
        <span className="w-8 h-8 bg-red-500/20 text-red-400 rounded-lg flex items-center justify-center text-xs font-bold">▶</span>
        End Shift & Tally
      </button>
    </div>
  );
    ...
    <div className="relative h-full w-full bg-gray-100 flex flex-col overflow-hidden">
      ...
      <div className="absolute top-0 inset-x-0 z-20 p-4 flex items-center justify-between pointer-events-none">
        ...
        <div className="flex gap-2 pointer-events-auto">
          ...
          {/* Modern Subtle Panic Button */}
          <button className="w-10 h-10 rounded-full bg-gray-900/50 backdrop-blur-md border border-white/20 flex items-center justify-center text-gray-300 hover:bg-red-500 hover:border-red-400 hover:text-white hover:shadow-lg hover:shadow-red-500/30 transition-all duration-300 group">
      ...
      <div className="md:hidden absolute inset-0 z-10 flex flex-col pointer-events-none">
        ...
        {/* BOTTOM SHEET (Dark Sidebar Theme) */}
        <div className={`absolute bottom-0 inset-x-0 z-20 bg-[#050F1A]/95 backdrop-blur-xl rounded-t-3xl border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] transition-transform duration-300 ease-in-out pointer-events-auto mt-auto ${
          ...
          <div className="absolute -top-24 right-6 z-30">
            <button className="w-20 h-20 rounded-2xl bg-[#1A5FB4] flex items-center justify-center shadow-2xl shadow-blue-500/40 border-4 border-[#050F1A] hover:bg-[#165a9f] transition-colors group">
          ...
          <div className="h-16 flex flex-col items-center justify-center cursor-pointer flex-shrink-0" onClick={() => setShowMobileSheet(!showMobileSheet)}>
            <div className={`w-10 h-1 rounded-full transition-colors duration-300 ${showMobileSheet ? "bg-white/20" : "bg-white/40"}`} />
            {!showMobileSheet && (
              <span className="text-[10px] font-semibold text-white/40 mt-2 uppercase tracking-wider">Shift Status</span>
            )}
          </div>
      ...
      {/* ================= DESKTOP LAYOUT ================= */}
      {/* Changed from inset-0 to bottom-4 right-4. Removed stretching classes so it fits content exactly. */}
      <div className="hidden md:block absolute bottom-4 right-4 z-10 pointer-events-auto w-[380px] bg-[#050F1A]/95 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl overflow-hidden">
