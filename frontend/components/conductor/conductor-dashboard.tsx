...
export default function ConductorDashboard() {
  ...
  // --- SHARED UI (Takes showHeader prop for Desktop only) ---
  const DashboardCardContent = ({ showHeader = false }: { showHeader?: boolean }) => (
    <>
      {/* THIS HEADER ONLY RENDERS ON DESKTOP */}
      {showHeader && (
  ...
  return (
    <div className="relative h-full w-full bg-[#050F1A]">
      ...
      {/* ========================================== */}
      {/* --- MOBILE UI (UNTOUTCHED) --- */}
      {/* ========================================== */}
      
      {/* Mobile Top Bar */}
      <div className="lg:hidden absolute top-0 inset-x-0 z-20 p-4 flex items-start justify-between pointer-events-none">
        ...
        <div className="flex gap-2 pointer-events-auto">
          <button className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-md border border-white/10 flex items-center justify-center text-red-400 hover:bg-red-500 hover:text-white hover:border-red-400 transition-all">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" /></svg>
          </button>
          <div className="w-10 h-10 rounded-full bg-[#1A5FB4] flex items-center justify-center text-white font-bold shadow-lg border-2 border-white/20">{mockData.name[0]}</div>
        </div>
      ...
      {/* Mobile Bottom Sheet (No showHeader passed, so it defaults to false) */}
      <div className="lg:hidden absolute bottom-0 inset-x-0 z-20 bg-[#071A2E]/95 backdrop-blur-xl rounded-t-3xl border-x-0 border-b-0 border-t border-white/10 shadow-2xl overflow-hidden pointer-events-auto">
        <div 
          className="h-12 flex flex-col items-center justify-center cursor-pointer flex-shrink-0" 
          onClick={() => setShowSheet(!showSheet)}
        >
          <div className={`w-10 h-1 rounded-full transition-colors duration-300 ${showSheet ? "bg-white/30" : "bg-white/60"}`} />
        </div>

        <div className={`transition-all duration-300 ease-in-out overflow-y-auto ${showSheet ? "max-h-[80vh] opacity-100" : "max-h-0 opacity-0"}`}>
      ...
      {/* Desktop Floating Card (showHeader={true} passed here) */}
      <div className="hidden lg:block absolute bottom-4 right-4 z-10 pointer-events-auto w-[380px] bg-[#071A2E]/95 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl overflow-hidden">
        <DashboardCardContent showHeader={true} />
      </div>
      ...
      <FareCalculatorModal 
        ...
        onRequestQrPayment={() => setShowQrPayment(true)}
      />
