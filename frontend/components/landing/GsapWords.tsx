...
interface GsapWordsProps {
  text: string;
  className?: string;
  /** Words matching these (lowercased, punctuation-stripped) get the accent gradient. */
  accentWords?: string[];
  as?: "p" | "h2" | "h3";
}
 ...
 *
 * Falls back to fully-visible text when the user prefers reduced motion (the
 * fromTo never runs, so words stay at their natural CSS state).
 */
          ...
          <span key={i} className="inline-block overflow-hidden align-bottom mr-[0.28em]">
            <span
              ...
              className={
                ...
                (isAccent
                  ? " bg-gradient-to-r from-[#62A0EA] to-[#99C1F1] bg-clip-text text-transparent"
                  : "")
              }
            >
              {word}
