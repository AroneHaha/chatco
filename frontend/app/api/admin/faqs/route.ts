...
export async function GET(request: NextRequest) {
  const result = await proxyToLaravel(request, "/admin/faqs", { method: "GET" });
  if (!result.ok) return jsonError(result.message ?? "Failed to load FAQs.", result.status);
...
export async function POST(request: NextRequest) {
  ...
  const result = await proxyToLaravel(request, "/admin/faqs", { method: "POST", body });
  if (!result.ok) {
    if (result.status === 422) return jsonValidationError(result.message ?? "Validation failed.", result.errors, 422);
    return jsonError(result.message ?? "Failed to create FAQ.", result.status);
  }
