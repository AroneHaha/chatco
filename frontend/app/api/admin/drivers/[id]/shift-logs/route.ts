...
/**
 * GET /api/admin/drivers/{id}/shift-logs
 * Proxies to Laravel GET /api/v1/admin/drivers/{id}/shift-logs.
 * Paginated (?page=, ?per_page=) — powers the driver detail modal's
...
export async function GET(
  ...
  const result = await proxyToLaravel(
    request,
    `/admin/drivers/${id}/shift-logs${request.nextUrl.search}`,
    { method: "GET" }
  );
