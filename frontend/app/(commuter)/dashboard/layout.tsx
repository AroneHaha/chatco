...
export default function CommuterLayout({ children }: { children: React.ReactNode }) {
  ...
  const [activeDotIndex, setActiveDotIndex] = useState(
    navItems.findIndex((item) => item.href === pathname) // Uses navItems
  );
    ...
    <div className="fixed inset-0 bg-black flex flex-col overflow-hidden font-sans">
      ...
      <nav className="absolute bottom-0 inset-x-0 z-50 bg-[#071A2E]/95 backdrop-blur-xl border-t border-white/10">
        <div className="relative flex items-center justify-around h-20 px-2 max-w-lg mx-auto">
          ...
          {navItems.map((item, index) => { // Uses navItems
...
// --- 2. NAV ITEMS ARRAY (Defined last, so it can use the icons above) ---
const navItems = [
