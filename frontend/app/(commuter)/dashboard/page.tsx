...
export default function CommuterHome() {
  ...
  return (
    <div className="relative h-full w-full bg-[#050F1A] flex flex-col">
      ...
      <div className={`absolute bottom-0 inset-x-0 z-20 bg-[#071A2E] rounded-t-3xl border-t border-white/10 transition-transform duration-300 ease-in-out ${
        ...
        {/* HAIL ME BUTTON (Anchored exactly above this panel) */}
        <div className="absolute -top-23 right-4 z-30">
          <button 
            onClick={() => setIsHailing(!isHailing)}
          ...
          >
            {isHailing ? (
              <svg className="w-7 h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
            ) : (
          ...
          <span className="block text-center text-[10px] font-extrabold text-white mt-2 drop-shadow-lg uppercase tracking-wider">
            {isHailing ? "Cancel" : "Hail Me"}
          </span>
        </div>

        {/* The 64px handle area */}
        <div 
        ...
        >
          ...
          {!showSheet && (
            <div className="flex items-center gap-1.5 mt-2 text-white/50">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-wider">Wallet & Tools</span>
            </div>
          )}
        </div>
